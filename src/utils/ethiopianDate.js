...
export function toEthiopianDate(dateInput = new Date()) {
  ...
  if (isNaN(date.getTime())) {
    return { year: 2018, month: 1, day: 1, monthNameAm: 'መስከረም', monthNameEn: 'Meskerem' };
  }
  ...
  const ey = 4 * Math.floor((jdn - ethJdnOffset) / 1461) + Math.floor(r / 365) - Math.floor(r / 1460);
...
export function fromEthiopianDate(year, month, day) {
  ...
  for (let offset = -30; offset <= 400; offset += 1) {
    ...
    if (converted.year === year && converted.month === month && converted.day === day) {
