...
export default function Grades() {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        // Filter for Young track by default
        const youngSections = secRes.data?.filter(s => s.program_type?.name === 'Young') || [];
        setSections(youngSections);
        if (youngSections.length > 0) setSelectedSection(youngSections[0].id);

        const youngCourses = courseRes.filter(c => c.program_type?.name === 'Young') || [];
        setCourses(youngCourses);
      } catch (err) {
  ...
  useEffect(() => {
    const fetchStudents = async () => {
      ...
      setLoading(true);
      try {
        const res = await sectionService.getSectionStudents(selectedSection);
        setStudents(res || []);
      } catch (err) {
  ...
  useEffect(() => {
    const fetchSectionCourses = async () => {
      ...
      try {
        const res = await sectionService.getSectionCourses(selectedSection);
        const assigned = Array.isArray(res) ? res : [];
        setSectionCourses(assigned);

        // Keep selected course only if it is assigned to current section.
        const assignedIds = new Set(assigned.map((c) => String(c.id)));
        if (!assignedIds.has(String(selectedCourse))) {
          setSelectedCourse(assigned.length > 0 ? String(assigned[0].id) : '');
        }
      } catch (err) {
      ...
      try {
        ...
        courseAssessments.forEach((assessment) => {
          (assessment.grades || []).forEach((grade) => {
            ...
            byStudent[grade.student_id][assessment.assessment_id] = Number(grade.score);
          });
        });
  ...
  const handleGradeChange = (studentId, assessment, value) => {
    ...
    if (numVal !== '' && (Number.isNaN(numVal) || numVal < 0 || numVal > Number(assessment.max_score))) return;
  ...
  const handlePublishGrades = async () => {
    ...
    setMessage('');
    try {
      await Promise.all(payloads.map((payload) => api.post('/grades', payload)));
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">
            {isSuperAdmin ? 'View configured assessments and grade sheets' : 'Manage grading sheets and compute final scores'}
          </p>
        </div>
        <button
          type="button"
      ...
      {assessments.length > 0 && (
        <div className={`text-sm font-bold px-4 py-3 rounded-xl border ${Math.round(assessmentsWeightTotal) === 100 ? 'text-green-700 bg-green-50 border-green-200' : 'text-amber-700 bg-amber-50 border-amber-200'}`}>
          Assessment weights total: {Number(assessmentsWeightTotal.toFixed(2))}%.
          {Math.round(assessmentsWeightTotal) !== 100 ? ' Grades are normalized to /100 automatically.' : ''}
        </div>
      )}
      ...
      <div className="glass-panel p-4 flex flex-wrap sm:flex-nowrap gap-8 items-center border-b-[3px] border-b-brand-500">
         <div className="flex items-center gap-3">
            ...
            <div>
               <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Section</label>
               <select 
         ...
         <div className="flex items-center gap-3 mr-auto">
            ...
            <div>
               <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Active Course</label>
               <select 
                 className="bg-transparent border-none outline-none font-bold text-slate-700 p-0 cursor-pointer hover:text-brand-600 transition-colors" 
      ...
      <div className="glass-panel overflow-hidden">
        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-slate-100">
            {loading ? (
              <tr><td colSpan={Math.max(3, assessments.length + 3)} className="px-6 py-12 text-center text-slate-400 font-bold animate-pulse">Loading...</td></tr>
            ) : filteredStudents.length === 0 ? (
              <tr><td colSpan={Math.max(3, assessments.length + 3)} className="px-6 py-12 text-center text-slate-400 font-bold">No students found in this section</td></tr>
            ) : availableCourses.length === 0 ? (
              <tr><td colSpan={Math.max(3, assessments.length + 3)} className="px-6 py-12 text-center text-slate-400 font-bold">No course is assigned to this section</td></tr>
            ) : assessments.length === 0 ? (
              <tr><td colSpan={Math.max(3, assessments.length + 3)} className="px-6 py-12 text-center text-slate-400 font-bold">No assessments configured for this course</td></tr>
            ) : (
              filteredStudents.map(s => {
                ...
                return (
                  <tr key={s.id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    {assessments.map((assessment) => (
                      <td key={`${s.id}-${assessment.assessment_id}`} className="px-6 py-4">
                        <input
                          type="number"
                          ...
                          onChange={(e) => handleGradeChange(s.id, assessment, e.target.value)}
