...
export default function LiveAttendance() {
  ...
  const canTakeLive =
    !isSuperAdmin && (isTmhrt || isMezmur);

  const canLiveForAssignment = (assign) => {
    if (!assign || !canTakeLive) return false;
    if (isTmhrt && assign.type === 'Course') return true;
    if (isMezmur && assign.type === 'MezmurTraining') return true;
    return false;
  };
  ...
  const fetchHistory = async (page = 1) => {
    ...
    try {
      const res = await attendanceService.getAttendanceRecords({ page, per_page: 25 });
  ...
  const fetchSessionData = async () => {
    ...
    try {
      ...
      try {
        const assign = await assignmentService.getAssignments({ q: assignmentId, per_page: 1 });
        currentAssign = Array.isArray(assign?.data)
          ? assign.data.find((a) => String(a.id) === String(assignmentId))
          : null;
      } catch (searchErr) {
        console.warn('Assignment search failed, falling back to list fetch.', searchErr);
      ...
      if (!currentAssign) {
        setStudents([]);
        setScanMessage('Assignment not found. Open it from Schedules or history.');
        return;
      }
      ...
      setScanMessage(err.response?.data?.message || 'Failed to load attendance session.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const liveMode = Boolean(assignmentId && assignment && canLiveForAssignment(assignment));
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">
            ...
            {isMezmur && !isTmhrt && !isSuperAdmin && 'Mezmur training attendance only.'}
            {hasRole('teacher') && !isTmhrt && !isMezmur && !isSuperAdmin && 'Course attendance you can view.'}
          </p>
        </div>
                ...
                {historyLoading ? (
                  ...
                  historyData.map((row) => (
                    <tr
                      ...
                      onClick={() => navigate(`/attendance?assignment_id=${row.assignment_id}`)}
                    >
                      ...
                      <td className="px-4 py-3 text-slate-700 max-w-[200px] truncate" title={assignmentLabel(row.assignment)}>
                        {assignmentLabel(row.assignment)}
                      </td>
                      <td className="px-4 py-3 text-xs font-bold text-slate-500 uppercase">
                        {row.assignment?.type === 'MezmurTraining' ? 'Mezmur' : 'Course'}
                      </td>
                      <td className="px-4 py-3 text-slate-600">{row.marked_by?.name || '—'}</td>
                    </tr>
                  ))
                )}
              </tbody>
        ...
        <p className="text-xs text-slate-400 font-medium">
          ...
          {canTakeLive && ' Use Schedules to take live attendance for your programs.'}
        </p>
      </div>
    );
  }
  ...
  if (!assignment && loading) {
    return (
      <div className="py-20 text-center text-slate-400 font-bold animate-pulse">Loading session…</div>
    );
  }
          ...
          <div className="flex bg-white p-1 rounded-2xl shadow-sm border border-slate-200">
            <button
              ...
              className={`px-5 py-2.5 text-sm font-bold rounded-xl flex items-center gap-2 transition-all ${
                mode === 'grid' ? 'bg-brand-50 text-brand-600 shadow-sm' : 'text-slate-500 hover:bg-slate-50'
              }`}
            >
            ...
            <button
              ...
              onClick={() => setMode('qr')}
              className={`px-5 py-2.5 text-sm font-bold rounded-xl flex items-center gap-2 transition-all ${
                mode === 'qr' ? 'bg-brand-50 text-brand-600 shadow-sm' : 'text-slate-500 hover:bg-slate-50'
              }`}
            >
      ...
      {liveMode && mode === 'qr' ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
          <div className="glass-panel overflow-hidden p-8 border-brand-200 bg-brand-50/10">
            ...
            {scanMessage && (
              <p className="mt-4 text-center text-xs font-bold text-slate-600">{scanMessage}</p>
            )}
          </div>
