...
export default function TeachersManagement() {
  ...
  const canEdit = hasRole("tmhrt_kfl") || hasRole("super_admin");
    ...
    <div className="space-y-6 animate-[fade-in_0.5s_ease-out]">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">
            Manage academic staff for the Young Program Track
          </p>
        </div>
        {canEdit && (
          <button
            onClick={() => openModal()}
            className="flex items-center gap-2 bg-linear-to-r from-brand-700 to-brand-500 text-white px-5 py-2.5 rounded-xl font-bold shadow-lg shadow-brand-500/30 hover:-translate-y-0.5 transition-all"
          >
      ...
      <div className="glass-panel p-2 flex flex-col md:flex-row gap-4 items-center bg-slate-50 border-b border-slate-100">
        ...
        <button
          ...
          className="p-3 bg-white border border-slate-200 rounded-2xl text-slate-400 hover:text-brand-500 hover:border-brand-200 transition-all shadow-sm"
        >
          {loading ? (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
        {teachers.length === 0 && !loading ? (
          ...
          teachers.map((teacher) => (
            <div
              key={teacher.id}
              className="group glass-panel p-6 bg-white hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-300 border border-slate-100 relative overflow-hidden flex flex-col justify-between h-full"
            >
                ...
                <div className="flex gap-2">
                  {canEdit && (
                    <button
                      ...
                      className="p-2 bg-brand-50 text-brand-600 rounded-lg hover:bg-brand-600 hover:text-white transition-all shadow-sm"
                    >
              ...
              <div className="flex items-center gap-4 mb-6">
                <div className="w-14 h-14 bg-linear-to-br from-brand-50 to-brand-100 rounded-2xl flex items-center justify-center text-brand-600 font-black text-xl shadow-inner border border-brand-200/50">
                  {teacher.name.charAt(0).toUpperCase()}
                </div>
                <div className="overflow-hidden">
                  <h3 className="font-extrabold text-slate-800 text-lg truncate group-hover:text-brand-600 transition-colors">
                    {teacher.name}
                  </h3>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {teacher.program_types?.map((pt) => (
                      <span
                        key={pt.id}
                        className="px-2.5 py-1 bg-brand-50 text-brand-700 rounded-lg text-[10px] font-black uppercase tracking-tight border border-brand-100"
                      >
                        {pt.name}
                      </span>
                    ))}
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 font-sans">
          <div className="bg-white rounded-4xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto transform animate-slide-up">
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-8">
              <div className="bg-slate-50/50 p-6 rounded-3xl border border-slate-100 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest block mb-1.5 ml-1">
                      Password {selectedTeacher && "(Optional)"}
                    </label>
                    <div className="relative group">
              ...
              <div className="pt-4">
                <button
                  type="submit"
                  className="w-full py-5 bg-brand-600 text-white rounded-3xl font-black uppercase tracking-widest hover:bg-brand-700 transition-all shadow-2xl shadow-brand-500/30 active:scale-[0.98] flex items-center justify-center gap-3"
                >
