...
export default function AssignmentsTasks() {
  ...
  const fetchResources = async () => {
    ...
    // Young sections only
    setSections(
      (secData?.data ?? []).filter((s) => s.program_type?.name === "Young"),
    );
    ...
    const allTrainers = Array.isArray(trainerData)
      ? trainerData
      : (trainerData?.data ?? []);
  ...
  const visibleEvents = useMemo(() => {
    ...
    return events.filter((ev) => {
      ...
      if (isAcademicAdmin) {
        return ev.type === "Course";
      }

      if (isMezmurAdmin) {
        return ev.type === "MezmurTraining";
      }

      if (isTeacher) {
        return ev.raw?.user_id === user?.id;
      }

      return false;
    });
  }, [events, user, hasRole, isSuperAdmin, isAcademicAdmin, isMezmurAdmin]);

  // Modal title/subtitle per role
  const modalTitle = isMezmurAdmin
    ? "New Mezmur Training"
  ...
  const inputCls =
    "w-full p-4 bg-white border border-slate-200 rounded-2xl font-bold text-slate-700 outline-none focus:border-brand-500 shadow-sm";
        ...
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">
            {isAcademicAdmin
              ...
              : isMezmurAdmin
                ? "Mezmur training schedule"
                : "Academic & ministerial schedule"}
          </p>
        </div>
        ...
        <div className="flex bg-white p-1 rounded-xl shadow-sm border border-slate-100 w-full md:w-auto">
          {[
            ...
            <button
              key={tab.key}
              onClick={() => {
                setActiveTab(tab.setV ? "calendar" : "list");
                if (tab.setV) setView(tab.setV);
              }}
              className={`flex-1 md:flex-none px-4 py-2 text-[10px] font-bold uppercase tracking-widest rounded-lg flex items-center justify-center gap-2 transition-all ${
                (
                  tab.setV
                    ? activeTab === "calendar" && view === tab.setV
                    : activeTab === "list"
                )
          ...
          <div className="space-y-4">
            {visibleEvents.length === 0 ? (
              ...
              visibleEvents.map((event) => (
                <div
                  key={event.id}
                  className="flex flex-col md:flex-row md:items-center justify-between p-5 border border-slate-100 rounded-2xl hover:border-brand-200 hover:shadow-lg transition-all gap-4 bg-slate-50/30"
                >
                  <div className="flex items-start gap-4">
                    <div
                      className={`w-12 h-12 rounded-2xl flex items-center justify-center shrink-0 shadow-inner ${event.type === "Course" ? "bg-brand-50 text-brand-600" : "bg-amber-50 text-amber-600"}`}
                    >
                      {event.type === "Course" ? (
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {event.raw?.scheduled_date && (
                          <span className="px-2 py-0.5 bg-amber-100 text-amber-700 text-[9px] font-black uppercase rounded-md tracking-tighter">
                            One-time
                          </span>
                        )}
                      </div>
                      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs font-bold text-slate-400 uppercase tracking-wider">
                        <span className="flex items-center gap-1.5">
                          ...
                          {event.raw?.start_time} - {event.raw?.end_time}
                        </span>
                        <span className="flex items-center gap-1.5">
                          <User className="w-3.5 h-3.5" /> {event.teacher}
                        </span>
                        ...
                        setFormData({
                          ...
                          day_of_week:
                            val === "periodic"
                              ...
                              : "",
                          scheduled_date:
                            val === "one-time" ? formData.scheduled_date : "",
                        })
                      }
