...
export default function EthiopianDateInput({
  ...
  const chooseDay = (day) => {
    onChange({
      target: { value: fromEthiopianDate(view.year, view.month, day) },
    });
    setOpen(false);
  };
  ...
  const yearOptions = Array.from(
    { length: 151 },
    (_, index) => today.year - 100 + index,
  );
  const displayValue = value
    ? getDualDateString(`${value}T00:00:00`, "am")
    : "Select an Ethiopian date";
  ...
  return (
    <div ref={wrapperRef} className="relative">
      ...
      >
        ...
        <span
          className={`text-sm font-bold ${value ? "text-slate-800" : "text-slate-400"}`}
        >
          {displayValue}
        </span>
      </button>
          ...
          <div className="flex items-center justify-between gap-2 mb-4">
            ...
            <div className="flex items-center gap-2">
              <select
                ...
                className="bg-brand-50 text-brand-800 rounded-lg px-2 py-1.5 text-sm font-black outline-none"
              >
              ...
              <select
                ...
                className="bg-slate-100 text-slate-800 rounded-lg px-2 py-1.5 text-sm font-black outline-none"
              >
                ...
                (() => {
                  ...
                  return (
                    <button
                      ...
                      className={`h-9 rounded-lg text-sm font-bold transition-colors ${isSelected ? "bg-brand-600 text-white shadow-sm" : isToday ? "border border-brand-400 text-brand-700 bg-brand-50" : "text-slate-700 hover:bg-brand-50 hover:text-brand-700"}`}
                    >
                      {day}
                    </button>
                  );
